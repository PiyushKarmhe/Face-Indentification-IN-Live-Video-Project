...
while True:
    ...
    path="C:\\Users\\PIYUSH KARMHE\\Documents\\C++ codes\\image_dir\\"
    ...
    if x==0:
        for (x, y, w, h) in faces:
            cv2.putText(imgc, 'PIYUSH', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
    elif x==1:
        for (x, y, w, h) in faces:
            cv2.putText(imgc, 'ANGELINA', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
    cv2.imshow('img', imgc)
    key = cv2.waitKey(1)
